...
class DataValidation:
    ...
    def inititiate_data_validation(self) ->DataValidationArtifact:
        try:
            ...
            # checking the column len of dataframe for train and test data
            status = self.validate_number_of_columns(dataframe=train_df)
            if not status:
                validation_error_msg += f"Columns are missing in training data"
            else:
                logging.info(f"All required columns are present in training dataframe. ")
            ...
            if not status:
                ...
                logging.info(f"All required columns are present in test dataframe. ")

            #validating column data type for train and test dataframe
            status = self.is_column_exist(df=train_df)
            if not status:
                validation_error_msg += f"COlumns are missing in traing_df"
            else:
